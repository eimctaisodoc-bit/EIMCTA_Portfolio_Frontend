...
const AdminFeature = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/adv/${_id}`, formData, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  const handleInputChange = (e, index = null, type = null) => {
    ...
    console.log('Checkbox:', inputType)
    // For checkbox → use checked (true/false)
    const inputValue = inputType === "checkbox" ? checked : value;
    console.log(inputValue)

    // For dynamic list (like highlightsList)
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div className="space-y-8 shadow-md p-5 rounded-md">
              ...
              `}>
                          {/* Checkmark SVG */}
                          <svg
                            className={`w-4 h-4 text-white transition-transform duration-200 ${formData.isPublic ? 'scale-100' : 'scale-0'}`}
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            strokeWidth={3}
                      ...
                      <label
                        htmlFor="isPublic-checkbox"
                        className="group flex items-center space-x-4 cursor-pointer select-none"
                      >
                        {/* Text Label */}
                        <span className={`text-lg font-medium transition-colors duration-200 ${formData.isPublic ? 'text-amber-900' : 'text-stone-600'}`}>
                          Public
                        </span>
                      </label>
                    </div>
